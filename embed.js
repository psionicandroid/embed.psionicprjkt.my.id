...
function embed() {
    ...
    const rawFileURL = fetchFromJsDelivr ? `https://cdn.jsdelivr.net/gh/${user}/${repository}@${branch}/${file}` : `https://raw.githubusercontent.com/${user}/${repository}/${branch}/${file}`;
...
`);
    ...
    const loadHLJS = typeof hljs != "undefined" && typeof hljs.highlightBlock != "undefined" ? Promise.resolve() : loadScript(HLJSURL);
    let loadHLJSNum;
    if (showLineNumbers) {
        // hljs-num should be loaded only after hljs is loaded
        loadHLJSNum = loadHLJS.then(() => (typeof hljs != "undefined" && typeof hljs.lineNumbersBlock != "undefined" ? Promise.resolve() : loadScript(HLJSNumURL)));
    }
    ...
    Promise.all(showLineNumbers ? [fetchFile, loadHLJS, loadHLJSNum] : [fetchFile, loadHLJS])
        .then((result) => {
